...
from rare_oracle import *
from rare_seeding_oracle import *
from dom_rare_seeding_oracle import *
from dom_reverse_rare_seeding_oracle import *
...
class EdgeOracles:
    ...
    def get_oracle_config(self):
        ...
        for oracle in pool:
            ...
            if oracle == 'rareness':
                self.oracle_pool.append(RareOracle())
            if oracle == 'tfidf':
                self.oracle_pool.append(RareSeedingOracle())
            if oracle == 'dom-tfidf':
                self.oracle_pool.append(DomRareSeedingOracle(self.config, self.target_prog))
            if oracle == 'dom-rev-tfidf':
                self.oracle_pool.append(DomRevRareSeedingOracle(self.config, self.target_prog))
            ...
            if oracle == 'meuzz':
                self.oracle_pool.append(MeuzzOracle(self.config, self.target_prog))
    ...
    def get_raw_data(self, raw_data_files):
        ...
        # Make singleton list out of argument if it is not already a list
    ...
    def oracle_add_features(self, key, features):
        cur_oracle = self.get_current_oracle()
        if cur_oracle.__repr__() == 'meuzz':
            cur_oracle.oracle_add_features(key, features)
    ...
    def find_edges_for_se(self, raw_data_files, se_cov_files = [], max_inputs = 3, heuristic = None, explored_cases = []):
        ...
        oracle_no_afl_dep = ["bug-potential", "avg-bug-potential", "meuzz"]
        cur_oracle = self.get_current_oracle()
