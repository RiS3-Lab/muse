...
class RandomOracle:
    def __init__(self, config, target):
        ...
        conf.read(config)
        self.batch_run_seed = int(conf.get("moriarty", "batch_run_input_num"))
        self.fuzz_timeout = random.randrange(50,60) # in seconds
        self.se_timeout = random.randrange(100,200)*self.batch_run_seed # in seconds
        self.se_timeout = self.se_timeout * 2 if self.batch_run_seed <= 2 else self.se_timeout
        ...
        if self._se_running:
            ...
            if (math.ceil(time.time()) - self._start_time >= self.se_timeout) or \
               se.is_explorers_alive() == False: #some times se terminates early:
                self._start_time = math.ceil(time.time())
                self.se_timeout = random.randrange(150,200)*self.batch_run_seed # in seconds
                self.se_timeout = self.se_timeout * 2 if self.batch_run_seed <= 2 else self.se_timeout
                self._invoke_se = False
                self._se_running = False
