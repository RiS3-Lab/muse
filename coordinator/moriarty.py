    ...
    def poke_switch_oracle(self):
        if self.switch_oracle.time_to_invoke_explorer():
            ...
            if len(deduplicated_list) > 0:
                ...
                records = self.se_factory.run(deduplicated_list, self.explorer_cov_file_list, self.batch_run_seed_num)
                # register the record to edge oracle(meuzz)
                for record in records:
                    self.edge_oracle.oracle_add_features(record[0], record[1])
                # print "len explored tests:", len(self.explored_tests)
