    ...
    def get_result(self, raw_data, max_results, edge_threshold=1.0):
        ...
        for e, raw in raw_data.iteritems():
            ...
            try:
                ...
                utils.error_msg("[tfidf computation wrong, fallback]")
                print stat
                stat['tfidf'] = 0.1
            stats.append(stat)
            if self.global_dom:
        ...
        if self.global_dom:
            ...
            # top_tfidf = itertools.islice(stats, max_results * 10)
            #this is for scalability
            top_tfidf = stats[:max_results*10]


        for stat in top_tfidf:
            self.prep_edge_dom(stat['edge_id'], stat)
            try:
                stat['dom-tfidf'] = stat['tfidf'] * math.log(1 + float(self.edge_dom_map[stat['edge_id']]['dom_num']))
                # oracle_info("dom for %s edge exist"%e)
            except KeyError:
                #fallback to tfidf 
                # oracle_info("dom for %s edge not exist, fallback to tfidf"%e)
                stat['dom-tfidf'] = stat['tfidf']
            # oracle_info("seed: %s, tfidf: %d, dom-tfidf: %d"%(stat['edge_id'],stat['tfidf'],stat['dom-tfidf'] ))

        top_dom_tfidf = sorted(top_tfidf, key=itemgetter('dom-tfidf'), reverse=True)
        result = {}
        for stat in top_dom_tfidf:
            edge_id = stat['edge_id']
            score = stat['dom-tfidf']
